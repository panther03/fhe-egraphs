...
fn egraph_from_seqn(innodes: &str, outnodes: &str, eqns: &str) -> (HashMap<String, Id>, EGraph<Prop, ()>, Id) {
    let mut egraph = EGraph::<Prop, ()>::default().with_explanations_enabled();
    ...
    for outnode in outnodes.split(" ") {
        outnodes_vec.push(*ckt_node_to_eclass.get(outnode).unwrap());
    }
    let concat_id = egraph.add(Prop::Concat(outnodes_vec));
    (ckt_node_to_eclass, egraph, concat_id)
}
...
fn main() {
    ...
    let mode: ExtractMode = args
        ...
        .expect("Invalid mode!");
    args.next();
    let timeout_seconds = 60;//args.next().expect("No timeout given").parse::<u64>().expect("Invalid timeout").min(60);
    ...
    let (ckt_node_to_eclass, start_egraph, concat_node) = egraph_from_seqn(innodes, outnodes, start.as_str());
    let start_expr = start_egraph.id_to_expr(concat_node);
    ...
    let network = match mode {
        ExtractMode::MD => {
            ...
            let (_,best_node) = extractor.find_best(runner.egraph.find(concat_node));
            let explanation = runner.egraph.explain_equivalence(&start_expr, &best_node); 
            println!("{}", explanation.get_string());
            recexpr_traversal(best_node, outnodes, &outnode_ids)
